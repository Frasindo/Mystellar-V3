...
function myadvertisements_install()
{
	...
	$db->write_query("CREATE TABLE `".TABLE_PREFIX."myadvertisements_advertisements` (
	  ...
	  PRIMARY KEY (`aid`), INDEX(`expire`,`unlimited`)
		) ENGINE=MyISAM");
...
function myadvertisements_uninstall()
{
	...
	// remove settings
	$db->delete_query('settings', 'name IN ( \'myadvertisements_disabled\',\'myadvertisements_sendpm\',\'myadvertisements_sendpmuid\')');
...
function myadvertisements_admin()
{
	...
	if($run_module == 'tools' && $action_file == 'myadvertisements')
	{
		if ($mybb->request_method == "post")
		{
			switch ($mybb->input['action'])
			{
				...
				case 'do_addadvertisement':
					...
					$insert_query = array('name' => $name, 'description' => $description, 'expire' => $expire, 'exemptgroups' => $exemptgroups, 'advertisement' => $advertisement, 'zone' => $zid, 'unlimited' => $unlimited, 'disabled' => $disabled);
				...
				case 'do_editadvertisement':
					...
					$update_query = array('name' => $name, 'description' => $description, 'exemptgroups' => $exemptgroups, 'advertisement' => $advertisement, 'zone' => $zid, 'disabled' => $disabled);
		...
		if (!$mybb->input['action'] || $mybb->input['action'] == 'zones' || $mybb->input['action'] == 'advertisements' || $mybb->input['action'] == 'addzone' || $mybb->input['action'] == 'editzone' || $mybb->input['action'] == 'addadvertisement' || $mybb->input['action'] == 'editadvertisement')
		{
			$page->add_breadcrumb_item($lang->myadvertisements, 'index.php?module=tools-myadvertisements');
		...
		else if ($mybb->input['action'] == 'advertisements')
		{
			$page->output_nav_tabs($sub_tabs, 'myadvertisements_advertisements');
			
			// table
			$table = new Table;
			$table->construct_header($lang->myadvertisements_name, array('width' => '20%'));
			$table->construct_header($lang->myadvertisements_description, array('width' => '30%'));
			...
			$query = $db->simple_select('myadvertisements_advertisements', '*');
			...
			while ($ad = $db->fetch_array($query))
			{
				...
				$table->construct_cell(intval($advertisement['views']), array('class' => 'align_center'));
				$table->construct_cell(intval($advertisement['clicks']), array('class' => 'align_center'));
