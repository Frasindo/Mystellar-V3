...
function get_user_info_func($xmlrpc_params, $isArray = false, $arr = array())
{
    ...
    else
    {
        ...
        {
            $input['user_id'] = $arr['user_id'];
            $input['user_name'] = $arr['user_name'];
        }
    }
    ...
    if($memprofile['birthday'])
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->date_of_birth),
            'value' => basic_clean("{$membday} {$membdayage}"),
        );
    }
    ...
    if ($mybb->settings[$prefix.'enabled'] == "1")
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->tyl_total_tyls_given),
            'value' => basic_clean("{$memprofile['tyl_unumtyls']} ({$tylpd_percent_total})"),
        );
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->tyl_total_tyls_rcvd),
            'value' => basic_clean("{$memprofile['tyl_unumrcvtyls']} ({$tylrcvpd_percent_total})"),
        );
    }
    ...
    if($memprofile['timeonline'] > 0)
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->timeonline),
            'value' => basic_clean($timeonline),
        );
    }
    if($mybb->settings['usereferrals'] == 1 && $memprofile['referrals'] > 0)
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->members_referred),
            'value' => basic_clean($memprofile['referrals']),
        );
    }
    if($memperms['usereputationsystem'] == 1 && $displaygroup['usereputationsystem'] == 1 && $mybb->settings['enablereputation'] == 1 && ($mybb->settings['posrep'] || $mybb->settings['neurep'] || $mybb->settings['negrep']))
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->reputation),
            'value' => basic_clean($memprofile['reputation']),
        );
    }
    if($mybb->settings['enablewarningsystem'] != 0 && $memperms['canreceivewarnings'] != 0 && ($mybb->usergroup['canwarnusers'] != 0 || ($mybb->user['uid'] == $memprofile['uid'] && $mybb->settings['canviewownwarning'] != 0)))
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->warning_level),
            'value' => basic_clean($warning_level.'%'),
        );
    }
    if($memprofile['website'])
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->homepage),
            'value' => basic_clean($memprofile['website']),
        );
    }
    ...
    $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->icq_number),
            'value' => basic_clean($memprofile['icq']),
        );
    $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->aim_screenname),
            'value' => basic_clean($memprofile['aim']),
        );
    $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->yahoo_id),
            'value' => basic_clean($memprofile['yahoo']),
        );
    $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->msn),
            'value' => basic_clean($memprofile['msn']),
        );
    ...
    while($customfield = $db->fetch_array($query))
    {
        ...
        if ($customfieldval)
        {
            ...
            $custom_fields_list_arr[] = array(
                'name'  => basic_clean($customfield['name']),
                'value' => basic_clean($customfieldval),
            );
        }
    }
    ...
    if($memprofile['signature'] && ($memprofile['suspendsignature'] == 0 || $memprofile['suspendsigtime'] < TIME_NOW))
    {
        ...
        $custom_fields_list_arr[] = array(
            'name'  => basic_clean($lang->users_signature),
            'value' => basic_clean($memprofile['signature']),
        );
    }
    ...
    if($mybb->user['uid'] > 0 && $mybb->user['ignorelist'] != "")
    {
        ...
        $isIgnored = in_array($memprofile['uid'], $ignore_list);
    }
    ...
    $xmlrpc_user_info = array(
		...
		'user_type'          => new xmlrpcval(check_return_user_type($memprofile['username']), 'base64'),
