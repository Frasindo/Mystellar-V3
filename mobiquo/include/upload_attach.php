...
function upload_attach_func($xmlrpc_params)
{
	...
	$maxsize = 0;
	$query = $db->query("
            SELECT maxsize FROM ".TABLE_PREFIX."attachtypes WHERE mimetype LIKE '%" . $_FILES['attachment']['type'] . "%';
        ");
	while($forum = $db->fetch_array($query))
    {
        $maxsize = $forum['maxsize'];
    }
    if($_FILES['attachment']['size'] > $maxsize * 1024)
    {
    	return xmlrespfalse($lang->error_uploadsize);
    }
	...
	if(empty($attachedfile)){
		return xmlrespfalse($lang->error_uploadfailed_php4);
	}
