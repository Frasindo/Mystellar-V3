...
function m_stick_topic_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($fid, "canmanagethreads"))
    {
...
function m_close_topic_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($fid, "canopenclosethreads"))
    {
        return tt_no_permission();
    }
...
function m_delete_topic_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($fid, "candeleteposts"))
    {
    ...
    //Add soft-delete-threads support
    if(intval($input['mode']) === 1 && intval($mobiquo_config['soft_delete']) === 1)
    {
        $moderation->soft_delete_threads(array($tid));
    }
    else if(intval($input['mode']) === 2)
    {
...
function m_get_report_post_func($xmlrpc_params){
    ...
    while($post = $db->fetch_array($query))
    {
        ...
        $can_delete = (is_super_admin($mybb->user['uid']) || is_moderator($post['fid'], "candeleteposts") || $can_delete == 1) && $mybb->user['uid'] != 0;
        ...
        $post_list[] = new xmlrpcval(array(
            ...
            'post_author_user_type'  => new xmlrpcval(check_return_user_type($post['postusername']), 'string'),
            ...
            'can_approve'       => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($post['fid'], "canmanagethreads"), 'boolean'),
            'can_move'          => new xmlrpcval(is_super_admin($mybb->user['uid']) ||is_moderator($post['fid'], "canmovetononmodforum"), 'boolean'),
...
function m_get_new_report_func($xmlrpc_params)
{
	...
	else
	{
		...
		{
			if(is_super_admin($mybb->user['uid']) || is_moderator($fid, "canmanagereportedposts"))
			{
				++$report_count;
			}
		}
...
function m_move_topic_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($moveto, "canmanagethreads") && !is_moderator($fid, "canmovetononmodforum"))
    {
        return tt_no_permission();
    }
    $newperms = forum_permissions($moveto);
    if($newperms['canview'] == 0 && !is_super_admin($mybb->user['uid']) && !is_moderator($fid, "canmovetononmodforum"))
    {
        return tt_no_permission();
    }
...
function m_merge_post_func($xmlrpc_params)
{
	...
	if(!is_super_admin($mybb->user['uid']) && !is_moderator_by_pids($postlist, "canmanagethreads"))
	{
		error_no_permission();
	}
...
function m_merge_topic_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($mergethread['fid'], "canmanagethreads"))
    {
        return tt_no_permission();
    }
...
function m_rename_topic_func($xmlrpc_params)
{
	...
	if(!is_super_admin($mybb->user['uid']) && !is_moderator($fid, "caneditposts"))
	{
		if($thread['closed'] == 1)
		{
...
function m_delete_post_func($xmlrpc_params)
{
    ...
    {
        if($forumpermissions['candeletethreads'] == 1 || is_moderator($fid, "candeletethreads") || is_super_admin($mybb->user['uid']))
        {
            //Add soft-delete-threads support
            if(intval($input['mode']) === 1 && intval($mobiquo_config['soft_delete']) === 1)
            {
    ...
    {
        if($forumpermissions['candeleteposts'] == 1 || is_moderator($fid, "candeleteposts") || is_super_admin($mybb->user['uid']))
        {
...
function m_approve_post_func($xmlrpc_params)
{
    ...
    if(!is_super_admin($mybb->user['uid']) && !is_moderator($post['fid'], "canmanagethreads"))
    {
        return tt_no_permission();
    }
    ...
    */
    ...
    while($thread = $db->fetch_array($query))
    {
        ...
        $topic_list[] = new xmlrpcval(array(
            ...
            'topic_author_user_type'  => new xmlrpcval(check_return_user_type($thread['username']), 'string'),
            ...
            'can_delete'        => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "candeleteposts"), 'boolean'),
            'can_close'         => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canopenclosethreads"), 'boolean'),
            'can_approve'       => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canopenclosethreads"), 'boolean'),
            'can_stick'         => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canmanagethreads"), 'boolean'),
            'can_move'          => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canmovetononmodforum"), 'boolean'),
			'can_merge'         => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canmanagethreads"), 'boolean'),
			'can_merge_post'    => new xmlrpcval(is_super_admin($mybb->user['uid']) || is_moderator($thread['fid'], "canmanagethreads"), 'boolean'),
            'can_ban'           => new xmlrpcval($mybb->usergroup['canmodcp'] == 1, 'boolean'),
...
function m_get_moderate_post_func($xmlrpc_params)
{
    ...
    while($post = $db->fetch_array($query))
    {
        ...
        $post_list[] = new xmlrpcval(array(
            ...
            'post_author_user_type'  => new xmlrpcval(check_return_user_type($post['username']), 'string'),
            'icon_url'          => new xmlrpcval(absolute_url($post['avatar']), 'string'),
