...
function get_thread_func($xmlrpc_params)
{
    ...
    // Get soft delete param
    $is_delete = (intval($thread['deletedposts']) === 1 ? true : false);
    ...
    while($post = $db->fetch_array($query))
    {
        ...
        process_post_attachments($post['pid'], $post, false, $attachment_list, $inline_attachment_list);
        ...
        $post_xmlrpc['is_delete'] = new xmlrpcval($is_delete, 'boolean');
        $post_xmlrpc = array(
            ...
            'inlineattachments'       => new xmlrpcval($inline_attachment_list, 'array'),
        );
...
function get_announcement_func($xmlrpc_params)
{
	...
	$xmlrpc_post = new xmlrpcval(array(
		...
		'post_author_user_type'  => new xmlrpcval(check_return_user_type($announcementarray['username']), 'string'),
