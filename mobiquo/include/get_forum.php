...
function processForum($forum)
{
    ...
    //Can post?
    if(isset($settings['tapatalk_forum_read_only']))
    {
    ...
    $can_post_reply = $permissions['canpostreplys'];
    if(!($forum['type'] == "f" && $forum['open'] != 0 && $mybb->user['uid'] > 0 && $mybb->usergroup['canpostthreads']) || in_array($forum['fid'], $read_only_forums) || !$can_post_reply)
    {
        $can_post = false;
    }
    ...
    $xmlrpc_forum = new xmlrpcval(array(
        ...
        'can_subscribe' => new xmlrpcval($forumpermissions[$forum['fid']]['canviewthreads'] == 1 && $forum['type'] == 'f', 'boolean'),
