...
elseif($mybb->input['action'] == "do_search" && $mybb->request_method == "post")
{
    ...
    if(isset($mybb->input['started_by']) && $mybb->input['started_by'])
    {
        $mybb->input['keywords'] = "";
    }
...
if($mybb->input['action'] == "results")
{
    ...
    else // Displaying results as posts
    {
        ...
        if(isset($mybb->input['started_by']) && $mybb->input['started_by'])
        {
            $input_uid = $mybb->input['uid'];
            $where_sql = "AND t.uid = $input_uid";
        }
        else
        {
            $where_sql = "";
        ...
        $query = $db->query("
            ...
            LEFT JOIN ".TABLE_PREFIX."banned b ON (p.uid = b.uid)
            WHERE p.pid IN (".$db->escape_string($search['posts']).") " . $where_sql . " 
            ORDER BY $sortfield $order
            LIMIT $start, $perpage
        ");
        while($post = $db->fetch_array($query))
