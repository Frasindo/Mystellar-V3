	...
	if($has_announcements == true)
	{
		...
		foreach ($announcementlist as $announce)
		{
			...
			$xmlrpc_topic = new xmlrpcval(array(
				...
				'topic_author_user_type'  => new xmlrpcval(check_return_user_type($announce['username']), 'string'),
                'icon_url'          => new xmlrpcval(absolute_url($icon_url), 'string'),
				'reply_number'      => new xmlrpcval(0, 'int'),
				'view_number'       => new xmlrpcval(0, 'int'),
			    'timestamp'   => new xmlrpcval($announce['startdate'], 'string'),
                'last_reply_time'   => new xmlrpcval(mobiquo_iso8601_encode($announce['startdate']), 'dateTime.iso8601'),
