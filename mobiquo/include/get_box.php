	...
	if($db->num_rows($query) > 0)
	{
		...
		//	if($folder == 2 || $folder == 3)
			{ // Sent Items or Drafts Folder Check
				...
				if(count($recipients['to']) > 1 || (count($recipients['to']) == 1 && count($recipients['bcc']) > 0))
				{
					foreach($recipients['to'] as $uid)
					{
						...
						if(!empty($user['username']))
						{
							...
							"user_type" => new xmlrpcval(check_return_user_type($user['username'])),
				...
				else if($message['toid'])
				{
					...
					if(!empty($tofromusername))
					{
						...
						"user_type" => new xmlrpcval(check_return_user_type($tofromusername)),
			...
			$new_message = array(
                ...
                'timestamp'       => new xmlrpcval($message['dateline'], 'string'),
			...
			if($msg_from !== null)
            {
				...
				$new_message['msg_from_user_type'] = new xmlrpcval(check_return_user_type($msg_from), 'string');
            }
