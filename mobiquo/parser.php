...
function parse_mycode_preg_replace_callback_1($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_list($matches[1]);}
function parse_mycode_preg_replace_callback_2($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_list($matches[2], $matches[1]);}
function parse_mycode_preg_replace_callback_3($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_img($matches[2]);}
function parse_mycode_preg_replace_callback_4($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_img($matches[4], array($matches[1], $matches[2]));}
function parse_mycode_preg_replace_callback_5($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_img($matches[3], array(), $matches[1]);}
function parse_mycode_preg_replace_callback_6($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_img($matches[5], array($matches[1], $matches[2]), $matches[3]);}
function parse_mycode_preg_replace_callback_7($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_video($matches[1], $matches[2]); }

function cache_mycode_preg_replace_callback_1($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_url($matches[1].$matches[2]);}
function cache_mycode_preg_replace_callback_2($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_url($matches[1]);}
function cache_mycode_preg_replace_callback_3($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_url($matches[1].$matches[2], $matches[3]);}
function cache_mycode_preg_replace_callback_4($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_url($matches[1], $matches[2]);}
function cache_mycode_preg_replace_callback_5($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_email($matches[1]);}
function cache_mycode_preg_replace_callback_6($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_email($matches[1], $matches[2]);}
function mycode_parse_post_quotes_callback1($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_post_quotes($matches[2], $matches[1]); }
function mycode_parse_post_quotes_callback2($matches){ $parser = new Tapatalk_Parser(); return $parser->mycode_parse_post_quotes($matches[2], $matches[1], true);}
...
class Tapatalk_Parser extends postParser {
	...
	*/
	function mycode_parse_quotes($message, $text_only=false)
	{
		...
		if($text_only == false)
		{
            ...
            //    "[quote]$1[/quote]\n"
            //);
            $message = preg_replace_callback("#\[quote=(?:&quot;|\"|')?(.*?)[\"']?(?:&quot;|\"|')?\](.*?)\[\/quote\](\r\n?|\n?)#si", 'mycode_parse_post_quotes_callback1', $message);
		...
		else
		{
            ...
            //    "\n{$lang->quote}\n--\n$1\n--\n"
            //);
            $message = preg_replace_callback("#\[quote=(?:&quot;|\"|')?(.*?)[\"']?(?:&quot;|\"|')?\](.*?)\[\/quote\](\r\n?|\n?)#si", 'mycode_parse_post_quotes_callback2', $message);

		}

        //while(preg_match($pattern[0], $message)/* || preg_match($pattern[1], $message)*/)
        //{
        //$message = preg_replace_callback("#\[quote=(?:&quot;|\"|')?(.*?)[\"']?(?:&quot;|\"|')?\](.*?)\[\/quote\](\r\n?|\n?)#si", 'mycode_parse_post_quotes_callback', $message);
        //}
	...
	*/
    ...
    function cache_mycode()
	{
        ...
        $standard_mycode['url_simple']['regex_callback'] = "#\[url\]([a-z]+?://)([^\r\n\"<]+?)\[/url\]#si";
        $standard_mycode['url_simple']['replacement'] = 'cache_mycode_preg_replace_callback_1';

        $standard_mycode['url_simple2']['regex_callback'] = "#\[url\]([^\r\n\"<]+?)\[/url\]#i";
        $standard_mycode['url_simple2']['replacement'] = 'cache_mycode_preg_replace_callback_2';

        $standard_mycode['url_complex']['regex_callback'] = "#\[url=([a-z]+?://)([^\r\n\"<]+?)\](.+?)\[/url\]#si";
        $standard_mycode['url_complex']['replacement'] = 'cache_mycode_preg_replace_callback_3';

        $standard_mycode['url_complex2']['regex_callback'] = "#\[url=([^\r\n\"<&\(\)]+?)\](.+?)\[/url\]#si";
        $standard_mycode['url_complex2']['replacement'] = 'cache_mycode_preg_replace_callback_4';

        $standard_mycode['email_simple']['regex_callback'] = "#\[email\](.*?)\[/email\]#i";
        $standard_mycode['email_simple']['replacement'] =  'cache_mycode_preg_replace_callback_5';

        $standard_mycode['email_complex']['regex_callback'] = "#\[email=(.*?)\](.*?)\[/email\]#i";
        $standard_mycode['email_complex']['replacement'] =  'cache_mycode_preg_replace_callback_6';
		...
		foreach($standard_mycode as $code)
		{
            ...
            else if(isset($code['regex_callback']))
            {
                $this->mycode_cache['standard_callback'][] = array('find' => $code['regex_callback'], 'replacement' => $code['replacement']);
            }
		}
	}
	...
	*/
	...
	function parse_mycode($message, $options=array())
	{
        ...
        foreach($this->mycode_cache['standard_callback'] as $regexCallback)
        {
            $message = preg_replace_callback($regexCallback['find'], $regexCallback['replacement'], $message);
        }
		...
		{
			$message = preg_replace_callback("#\s?\[list\](.*?)\[/list\](\r\n?|\n?)#si", 'parse_mycode_preg_replace_callback_1', $message);
		}

		// Replace lists.
		while(preg_match("#\[list=(a|A|i|I|1)\](.*?)\[/list\](\r\n?|\n?)#si", $message))
		{
			$message = preg_replace_callback("#\s?\[list=(a|A|i|I|1)\](.*?)\[/list\]#si",  'parse_mycode_preg_replace_callback_2', $message);
		}

		// Convert images when allowed.
		if($options['allow_imgcode'] != 0)
		{
			$message = preg_replace_callback("#\[img\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is", 'parse_mycode_preg_replace_callback_3', $message);
			$message = preg_replace_callback("#\[img=([0-9]{1,3})x([0-9]{1,3})\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",'parse_mycode_preg_replace_callback_4', $message);
			$message = preg_replace_callback("#\[img align=([a-z]+)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is", 'parse_mycode_preg_replace_callback_5', $message);
			$message = preg_replace_callback("#\[img=([0-9]{1,3})x([0-9]{1,3}) align=([a-z]+)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is", 'parse_mycode_preg_replace_callback_6', $message);
		}

		// Convert videos when allow.
		if($options['allow_videocode'] != 0)
		{
			$message = preg_replace_callback("#\[video=(.*?)\](.*?)\[/video\]#si", 'parse_mycode_preg_replace_callback_7', $message);
		}
